...
namespace Hector
{
  namespace Element
  {
    ...
    HorizontalQuadrupole::matrix( float eloss, float mp, int qp ) const
    {
      ...
      mat( 1, 1 ) = cos( omega );
      mat( 1, 2 ) = sin( omega ) * ( -sqrt( ke ) );
      mat( 2, 1 ) = sin( omega ) * ( 1./sqrt( ke ) );
      mat( 2, 2 ) = cos( omega );
      // Defocussing Twiss matrix
      mat( 3, 3 ) = cosh( omega );
      mat( 3, 4 ) = sinh( omega ) * sqrt( ke );
      mat( 4, 3 ) = sinh( omega ) * ( 1./sqrt( ke ) );
      mat( 4, 4 ) = cosh( omega );
      return mat;
    }
    ...
    VerticalQuadrupole::matrix( float eloss, float mp, int qp ) const
    {
      ...
      // Defocussing Twiss matrix
      mat( 1, 1 ) = cosh( omega );
      mat( 1, 2 ) = sinh( omega ) * sqrt( ke );
      mat( 2, 1 ) = sinh( omega ) * ( 1./sqrt( ke ) );
      mat( 2, 2 ) = cosh( omega );
      // Focussing Twiss matrix
      mat( 3, 3 ) = cos( omega );
      mat( 3, 4 ) = sin( omega ) * ( -sqrt( ke ) );
      mat( 4, 3 ) = sin( omega ) * ( 1./sqrt( ke ) );
      mat( 4, 4 ) = cos( omega );
      return mat;
    }
  }
