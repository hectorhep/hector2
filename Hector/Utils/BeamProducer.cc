...
namespace Hector {
  ...
  BeamProducer::params_t BeamProducer::GaussianParticleGun::parameters(float lim1, float lim2) {
    return BeamProducer::params_t(0.5 * (lim1 + lim2), 0.5 * (lim2 - lim1));
  }

  void BeamProducer::GaussianParticleGun::smearXi(float xi_mean, float xi_sigma) {
